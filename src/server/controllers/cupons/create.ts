...
export const createCoupon = async(req : Request,res : Response)=>{
    const  {code, expiration, limitUses, 
         percentageDiscount , 
        valueFixDiscount , minimumValue, active}  = req.body 
    ...
    const codeInUse = await CuponsModel.find({code: code})
    ...
    try {
      ...
      return res.status(200).json({
        message: 'cupom criado com sucesso', newCupom
      })
    } catch (error) {
      console.log(error)
