...
export const addView = async (req: Request, res: Response) => {
  ...
  const user = await getUserByToken(res, userToken) as unknown as userInterface
  ...
  if (!product) {
    return res.status(404).json({
      message: 'nao e possivel add nova view,  produto nao encontrado com esse id'
    })
  }
  try {
    ...
    return res.status(200).json({
      message: "view salvo com sucesso", newView,
    });
  } catch (error) {
